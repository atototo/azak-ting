...
블로그/SNS 홍보용 공개 프리뷰 링크 시스템입니다. UUID 기반 링크로 인증 없이 접근 가능합니다.

#### `/public/[linkId]` - 공개 종목 프리뷰
- **파일**: `app/public/[linkId]/page.tsx`
- **설명**: 로그인 없이 접근 가능한 종목 분석 페이지
- **인증**: 불필요 (UUID 링크로 접근 제어)
- **동적 라우팅**: UUID 기반 link_id (예: `/public/a7f3e9b2-4c8d-4a1e-9f2a-1b3c4d5e6f7a`)
- **주요 기능**:
  - 종목 기본 정보 및 차트
  - AI 종합 투자 리포트
  - 최근 뉴스 & AI 분석
...
- **제약사항**:
  ...
  - 리포트 업데이트 버튼 미표시
  - URL에 종목코드 노출 안 됨 (보안)
- **생성 방법**: 관리자가 `/admin/stocks` 페이지에서 "🔗 홍보 링크" 버튼 클릭

### 특수 페이지 (`/preview/*`)

내부 개발/테스트용 프리뷰 페이지입니다. `?token=xxx` 쿼리 파라미터로 인증합니다.

#### `/preview/dashboard` - 대시보드 프리뷰
- **파일**: `app/preview/dashboard/page.tsx`
- **설명**: 블로그용 대시보드 스크린샷 (내부 테스트용)
...
**공개 프리뷰 vs 특수 프리뷰 차이점**:
| 구분 | 공개 프리뷰 (`/public/*`) | 특수 프리뷰 (`/preview/*`) |
|------|-------------------------|--------------------------|
| **목적** | 블로그/SNS 홍보용 | 내부 개발/테스트용 |
| **인증** | 불필요 (UUID 링크) | 프리뷰 토큰 필요 |
| **접근 범위** | 특정 종목만 | 모든 페이지 |
| **관리** | 관리자 UI에서 생성/삭제 | 환경변수 관리 |
