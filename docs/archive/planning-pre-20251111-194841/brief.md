...
**Azak**는 뉴스가 주가에 미치는 영향을 AI(LLM + RAG)로 예측하여, 개인 투자자가 "지금 사야 하나, 팔아야 하나?"를 즉시 판단할 수 있도록 돕는 투자 의사결정 지원 시스템입니다.
...
**Azak**는 지속적으로 수집한 과거 뉴스-주가 패턴 데이터를 벡터 DB에 저장하고, 새로운 뉴스 발생 시 유사 사례를 검색하여 LLM이 종합 분석한 결과를 텔레그램으로 즉시 알려주는 시스템입니다.
...
이 Project Brief는 **Azak** 프로젝트의 전체 컨텍스트를 제공합니다.
